...
with avg_sal(avg_salary) as (select cast(avg(salary) as UNSIGNED) from emp)
select *
from emp e
...
select cast(avg(total_sales_per_store) as UNSIGNED) avg_sale_for_all_store
...
join   (select cast(avg(total_sales_per_store) as UNSIGNED) avg_sale_for_all_store
				from (select s.store_id, sum(s.cost) as total_sales_per_store
		  	  		from sales s
			  			group by s.store_id) x
	   ) avg_sales
